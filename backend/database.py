...
def _parse_db_url(url: str) -> str:
    """Normaliza a URL para usar pg8000 independente do formato recebido."""
    if not url:
        return url
    ...
    # Remove query params (ex: ?sslmode=require) — pg8000 usa connect_args
    url = url.split("?")[0]

    # Escapa @ na senha se necessário
...
engine = create_engine(
    database_url,
    connect_args={"ssl_context": True},  # SSL requerido sem verificação de cert
    pool_size=15,
    max_overflow=25,
    pool_pre_ping=True,
    pool_recycle=300,
